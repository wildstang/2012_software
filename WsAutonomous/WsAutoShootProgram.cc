...
WsAutoShootProgram::WsAutoShootProgram(void)
    : WsAutonomousProgram("WsAutoShootProgram")
    , a_done(false)
...
void WsAutoShootProgram::init(void)
{
    WsConfigFacade::instance()->getAutoShoot6Config(a_configVals);
...
bool
WsAutoShootProgram::run(void)
{
    WS_LOG_TRACE("Begin");
    
    WsAutoShootProgramStateType new_state = a_currentState;
    bool shoot_button = false;
    float flywheel_state = OI_FLYWHEEL_POT_PRESET;
    WsTypes::WsShooterPresetType preset = WsTypes::WS_SHOOTER_PRESET_KEY_BACK;
    ...
    switch (a_currentState)
    {
        case WS_AUTO_SHOOT_STATE_PRIME_FLY_1:
        {
            ...
            shoot_button = false;
            preset = WsTypes::WS_SHOOTER_PRESET_KEY_BACK;
...
bool
WsAutoShootProgram::isDone(void)
{
    return a_done;
}
